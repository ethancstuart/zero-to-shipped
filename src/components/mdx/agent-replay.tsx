...
export function AgentReplay({ steps, title, agentRole }: AgentReplayProps) {
  ...
  return (
    <div className="my-8 rounded-xl border border-[hsl(var(--border-base))] bg-[hsl(var(--bg-subtle))] p-6">
      <div className="mb-6">
        <h3 className="text-lg font-semibold text-[hsl(var(--fg))]">{title}</h3>
        <span className="text-sm text-[hsl(var(--fg-muted))]">Agent: {agentRole}</span>
      </div>
          ...
          >
            ...
            >
              <div className="flex items-center gap-3">
                <span className="text-xs font-mono text-[hsl(var(--fg-faint))]">
                  {step.stepNumber}
                </span>
                <span className="rounded px-1.5 py-0.5 text-xs font-medium uppercase text-[hsl(var(--fg-muted))]">
                  {step.type.replace('_', ' ')}
                </span>
              ...
              <span className="text-[hsl(var(--fg-faint))]">
                {expandedStep === step.stepNumber ? '−' : '+'}
              </span>
            </button>
            {expandedStep === step.stepNumber && (
              <div className="border-t border-[hsl(var(--border-base))] p-4">
                <pre className="mb-3 whitespace-pre-wrap text-sm text-[hsl(var(--fg-secondary))]">
                  {step.content}
                </pre>
                {step.metadata && (
                  <div className="mb-3 flex gap-4 text-xs text-[hsl(var(--fg-muted))]">
                ...
                {step.annotation && (
                  <div className="rounded-lg border border-[hsl(var(--border-base))] bg-[hsl(var(--bg-muted))] p-3 text-sm text-[hsl(var(--fg-muted))]">
                    <span className="font-medium text-[hsl(var(--fg-secondary))]">Note: </span>
                    {step.annotation}
                  </div>
                )}
              </div>
