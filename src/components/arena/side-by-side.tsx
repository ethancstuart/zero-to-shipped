...
export function SideBySide({ entries, voteCounts, challengeId }: SideBySideProps) {
  return (
    ...
    >
      {entries.map((entry) => (
        <div key={entry.id} className="rounded-xl border border-[hsl(var(--border-base))] bg-[hsl(var(--bg-subtle))] p-6">
          ...
          <div className="mb-6 grid grid-cols-2 gap-3">
            {entry.duration_seconds && (
              <div className="rounded-lg bg-[hsl(var(--bg-muted))] p-3">
                <div className="text-lg font-bold text-[hsl(var(--fg))]">
                  {Math.floor(entry.duration_seconds / 60)}m {entry.duration_seconds % 60}s
                </div>
                <div className="text-xs text-[hsl(var(--fg-muted))]">Duration</div>
              </div>
            )}
            {entry.lines_of_code && (
              <div className="rounded-lg bg-[hsl(var(--bg-muted))] p-3">
                <div className="text-lg font-bold text-[hsl(var(--fg))]">{entry.lines_of_code}</div>
                <div className="text-xs text-[hsl(var(--fg-muted))]">Lines of Code</div>
              </div>
            )}
            {entry.prompt_count && (
              <div className="rounded-lg bg-[hsl(var(--bg-muted))] p-3">
                <div className="text-lg font-bold text-[hsl(var(--fg))]">{entry.prompt_count}</div>
                <div className="text-xs text-[hsl(var(--fg-muted))]">Prompts</div>
              </div>
            )}
            {entry.iteration_count && (
              <div className="rounded-lg bg-[hsl(var(--bg-muted))] p-3">
                <div className="text-lg font-bold text-[hsl(var(--fg))]">{entry.iteration_count}</div>
                <div className="text-xs text-[hsl(var(--fg-muted))]">Iterations</div>
              </div>
            )}
          </div>

          {entry.result_score && (
            <div className="mb-4 text-sm text-[hsl(var(--fg-muted))]">
          ...
          <div className="mb-4 flex gap-3 text-xs">
            {entry.final_repo_url && (
              <a
                ...
                className="text-[hsl(var(--fg-muted))] underline hover:text-[hsl(var(--fg-secondary))]"
              >
                View code →
              </a>
            )}
