...
export default async function CertificatePage() {
  ...
  const [profileRes, progressRes] = await Promise.all([
    supabase.from("profiles").select("display_name, role_track, public_profile").eq("id", user.id).single(),
    supabase.from("module_progress").select("module_number, status, completed_at").eq("user_id", user.id),
  ]);
  ...
  return (
    <div className="mx-auto max-w-2xl text-center">
      ...
      <p className="mb-8 text-muted-foreground">
        You&apos;ve completed the Prototype Studio curriculum. Download your
        certificate below.
      </p>
      ...
      <div className="mb-8 rounded-xl border-2 border-primary/20 bg-card p-12">
        ...
        <h2 className="mb-1 text-3xl font-bold">Prototype Studio</h2>
      ...
      {profile.public_profile && (
        <div className="mt-6 flex justify-center">
          <ShareButtons
            ...
            title={`I just completed Prototype Studio! 🚀 All 16 modules done.`}
          />
        </div>
      )}
    </div>
